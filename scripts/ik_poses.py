...
Requiere move_group corriendo (ros2 launch kr10_r1100_2_moveit_config demo.launch.py).
Para cada pose:
...
Uso:  python3 scripts/ik_poses.py
"""
import json
import math
import os
...
def main():
    rclpy.init()
    node = IKClient("manipulator", "tool0", "base_link")
    out = {"group": node.group, "tip": node.tip, "base": node.base, "joints": JOINTS, "poses": {}}
    ...
    out_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "resultados")
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "ik_resultados.json")
